    ...
    class Texto extends CampoSimple{
        ...
        public function pintar(array $valor){
            if(empty($valor)){
                ...
                return parent::pintar($valor)."placeholder='$this->placeholder' minlength='$this->minlen' maxlength='$this->maxlen' size='$this->size' value='".$valor[$this->name]."'/> </label>";
            }else{
        ...
        public function validar(array $valor){
            if(!empty($this->patter)){
                if($valor[$this->name]!=null && preg_match($this->patter,$valor[$this->name])){
                    if(strlen($valor[$this->name])>$this->minlen && strlen($valor[$this->name])<$this->maxlen){
                        return true;
                    }else{
                        return false;
                    }
                }
            }elseif($valor[$this->name]!=null){
                if(strlen($valor[$this->name])>=$this->minlen && strlen($valor[$this->name])<=$this->maxlen){
                    return true;
                }else{
                    return false;
